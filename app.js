...
document.getElementById("form-login").addEventListener("submit", async (e) => {
  ...
  const accion = modoLoginEsRegistro
    ? supabase.auth.signUp({ email: correo, password: clave })
...
function leerFormularioManual() {
  ...
  let ocup = REF.ref_ocupaciones.find(o => o.nombre.toUpperCase() === (registro.ocupacion_nombre || ""));
...
document.getElementById("form-manual").addEventListener("submit", async (e) => {
  ...
  btn.disabled = true;
  const { data, error } = await supabase.from("solicitudes_arl").insert(registro).select("radicado").single();
  btn.disabled = false;
  if (error) {
...
function validarFilaMasiva(registro) {
  ...
  if (registro.eps && !REF.ref_eps.find(e => e.nombre.toUpperCase() === registro.eps)) errores.push("EPS no reconocida");
  if (registro.tipo_identificacion && !REF.ref_tipo_identificacion.find(t => t.nombre.toUpperCase() === registro.tipo_identificacion)) errores.push("Tipo de identificación no reconocido");
  if (registro.genero && !REF.ref_sexo.find(g => g.nombre.toUpperCase() === registro.genero)) errores.push("Género no reconocido");
  if (registro.codigo_alto_riesgo && !REF.ref_codigo_riesgo.find(r => r.nombre.toUpperCase() === registro.codigo_alto_riesgo)) errores.push("Código alto riesgo no reconocido");
  if (registro.codigo_sucursal && !REF.ref_sucursales.find(s => String(s.codigo) === String(registro.codigo_sucursal) || s.nombre.toUpperCase() === registro.codigo_sucursal)) errores.push("Código de sucursal no reconocido");

  const ocup = REF.ref_ocupaciones.find(o => o.nombre.toUpperCase() === (registro.ocupacion || ""));
...
function validarYMostrarMasivo() {
  ...
  const filasHtml = filasMasivas.map((r, idx) => {
    ...
    const estadoBadge = errores.length === 0
      ...
      : `<span class="badge err" title="${escapeAttr(errores.join('; '))}">${errores.length} error(es)</span>`;
    return `<tr data-fila="${idx}" class="${errores.length && !soloAlerta ? 'fila-error' : ''}">
      ...
      <td>${escapeAttr(r.ocupacion)}</td>
    </tr>`;
  }).join("");

  tabla.innerHTML = `<thead><tr><th>#</th><th>Estado</th><th>Radicado</th><th>Nombre</th><th>Identificación</th><th>Facultad</th><th>EPS</th><th>Ocupación</th></tr></thead><tbody>${filasHtml}</tbody>`;
...
document.getElementById("btn-guardar-masivo").addEventListener("click", async () => {
  ...
  const { data, error } = await supabase.from("solicitudes_arl").insert(registros).select("radicado");
...
async function cargarRegistros() {
  ...
  let consulta = supabase.from("solicitudes_arl").select("*").order("creado_en", { ascending: false });
...
function construirLineaTxt(r) {
  const tipoId = REF.ref_tipo_identificacion.find(t => t.nombre.toUpperCase() === (r.tipo_identificacion || "").toUpperCase());
  const sexo = REF.ref_sexo.find(s => s.nombre.toUpperCase() === (r.genero || "").toUpperCase());
  const estadoCivil = r.estado_civil
    ? REF.ref_estado_civil.find(e => e.nombre.toUpperCase() === r.estado_civil.toUpperCase())
    : null;
  const tipoSalario = REF.ref_tipo_salario.find(t => t.nombre.toUpperCase() === (r.tipo_salario || "").toUpperCase());
  const eps = REF.ref_eps.find(e => e.nombre.toUpperCase() === (r.eps || "").toUpperCase());
  const riesgo = REF.ref_codigo_riesgo.find(x => x.nombre.toUpperCase() === (r.codigo_alto_riesgo || "").toUpperCase());
  const ciudad = r.ciudad
    ? REF.ref_departamentos_ciudades.find(c => c.ciudad.toUpperCase() === r.ciudad.toUpperCase() && c.departamento.toUpperCase() === (r.departamento || "").toUpperCase())
    : null;
...
document.getElementById("btn-exportar-txt").addEventListener("click", async () => {
  ...
  await supabase.from("solicitudes_arl").update({ estado: "exportado" }).in("id", ids);
  cargarRegistros();
