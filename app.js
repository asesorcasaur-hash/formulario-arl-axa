...
// Fecha inicio de cobertura debe ser mínimo 1 día después de la Fecha Inicio
// de Práctica (si la práctica inicia el 14, la cobertura solo puede iniciar
// el 15 en adelante, nunca el mismo día).
function coberturaMinimaOk(fechaCoberturaISO, fechaPracticaISO) {
  if (!fechaCoberturaISO || !fechaPracticaISO) return true;
  const practica = new Date(fechaPracticaISO + "T00:00:00");
  const minimo = new Date(practica);
  minimo.setDate(minimo.getDate() + 1);
  const cobertura = new Date(fechaCoberturaISO + "T00:00:00");
  return cobertura >= minimo;
}

// Calcula la edad en años cumplidos a la fecha de hoy (fecha de la solicitud)
...
function revisarAlertaCobertura() {
  ...
  const practica = document.getElementById("campo-fecha_inicio_practica")?.value;
  const wrap = document.querySelector('[data-clave="fecha_inicio_cobertura"]');
  const alerta = wrap?.querySelector(".alerta-caja");
  if (!alerta) return;
  if (!coberturaMinimaOk(cobertura, practica)) alerta.classList.add("visible");
...
function validarFilaMasiva(registro) {
  ...
  if (registro.fecha_inicio_cobertura && registro.fecha_inicio_practica &&
      !coberturaMinimaOk(registro.fecha_inicio_cobertura, registro.fecha_inicio_practica)) {
    errores.push("Fecha cobertura debe ser mínimo 1 día después de Fecha Inicio Práctica (se permite, pero verificar)");
  }
