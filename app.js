...
let identificacionesEnBD = new Set(); // cédulas del archivo actual que ya existen guardadas
...
async function procesarArchivoMasivo(archivo) {
  ...
  identificacionesEnBD = new Set();
  const idsUnicos = [...new Set(filasMasivas.map(r => r.identificacion).filter(Boolean))];
  if (idsUnicos.length > 0) {
    const { data, error } = await sb.from("solicitudes_arl").select("identificacion").in("identificacion", idsUnicos);
    if (!error && data) data.forEach(f => identificacionesEnBD.add(String(f.identificacion)));
  }
...
function validarFilaMasiva(registro, contexto = {}) {
  ...
  if (contexto.yaExisteEnBD) errores.push("Esta identificación ya fue cargada antes (existe en registros guardados)");
