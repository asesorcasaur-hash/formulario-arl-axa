...
let identificacionesEnBD = new Map(); // cédula -> radicado ya existente en la base de datos
...
function construirFormularioManual() {
  ...
  CAMPOS_FORMULARIO.forEach(campo => {
    ...
    } else if (campo.tipo === "select-sucursal") {
      control = document.createElement("select");
      control.innerHTML = `<option value="">Seleccione…</option>` +
        REF.ref_sucursales.map(r => `<option value="${escapeAttr(r.codigo)}">${escapeAttr(r.codigo)} - ${escapeAttr(r.nombre)}</option>`).join("");
...
function leerFormularioManual() {
  ...
  if (registro.eps && normalizarTexto(registro.eps) === normalizarTexto("MAGISTERIO")) {
    registro.eps = "NO SUMINISTRADA";
  }
...
function excelFechaAISO(valor) {
  ...
  // Fechas en texto: SOLO se aceptan en formato exacto DÍA/MES/AÑO
  // (ej. "29/06/2003" o "29-06-2003"). Cualquier otro formato de texto se
  // rechaza (retorna null) en vez de dejar que el navegador lo adivine,
  // porque eso invierte día/mes sin avisar cuando el día es ≤ 12.
  const texto = String(valor).trim();
  const coincide = texto.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);
  if (!coincide) return null;
  const [, d, m, a] = coincide;
...
async function procesarArchivoMasivo(archivo) {
  ...
  const CAMPOS_FECHA = ["fecha_nacimiento", "fecha_inicio_practica", "fecha_final_practica", "fecha_inicio_cobertura"];

  filasMasivas = [];
  for (let i = filaEncabezados + 1; i < filas.length; i++) {
    const fila = filas[i];
    if (fila.every(v => String(v).trim() === "")) continue;
    const registro = { origen: "masivo", nit_empresa: "0", _fechasInvalidas: [] };
    Object.entries(mapaColumnas).forEach(([clave, idx]) => {
      let valor = fila[idx];
      if (CAMPOS_FECHA.includes(clave)) {
        const original = valor;
        valor = excelFechaAISO(original);
        if (valor === null && String(original ?? "").trim() !== "") {
          registro._fechasInvalidas.push(clave);
        }
      } else if (typeof valor === "string") {
        valor = valor.trim().toUpperCase();
      }
      if (clave === "identificacion" && valor) valor = String(valor).replace(/\s+/g, "");
      registro[clave] = valor === "" ? null : valor;
    });
    registro.nit_empresa = "0"; // el NIT siempre queda en 0, sin importar lo que traiga el Excel
    registro.valor_salario = "1750905"; // el valor salario siempre queda en 1.750.905, sin importar lo que traiga el Excel
    filasMasivas.push(registro);
  }

  // Consulta cuáles de las identificaciones de este archivo ya existen
  // guardadas en la base de datos, para poder avisar de cédulas duplicadas
  // (y mostrar el radicado del registro ya existente).
  identificacionesEnBD = new Map();
  const idsUnicos = [...new Set(filasMasivas.map(r => r.identificacion).filter(Boolean))];
  if (idsUnicos.length > 0) {
    const { data, error } = await sb.from("solicitudes_arl").select("identificacion, radicado").in("identificacion", idsUnicos);
    if (!error && data) data.forEach(f => identificacionesEnBD.set(String(f.identificacion), f.radicado));
  }

  validarYMostrarMasivo();

  const totalFechasInvalidas = filasMasivas.reduce((n, r) => n + (r._fechasInvalidas?.length || 0), 0);
  if (totalFechasInvalidas > 0) {
    mostrarToast(`${totalFechasInvalidas} fecha(s) no cumplen el formato día/mes/año. Esas filas no se pueden guardar hasta corregirlas (botón "Editar").`, "error");
  }
}

function validarFilaMasiva(registro, contexto = {}) {
  const errores = [];
  const req = (v) => v !== null && v !== undefined && String(v).trim() !== "";
  const CAMPOS_FECHA_ETIQUETAS = {
    fecha_nacimiento: "Fecha de Nacimiento", fecha_inicio_practica: "Fecha Inicio Práctica",
    fecha_final_practica: "Fecha Final Práctica", fecha_inicio_cobertura: "Fecha Inicio de Cobertura",
  };

  if (contexto.duplicadoEnArchivo) errores.push("Identificación duplicada: aparece más de una vez en este mismo archivo");
  if (contexto.yaExisteEnBD) {
    if (registro._duplicadoConfirmado) {
      errores.push(`Ya existía con el radicado ${contexto.radicadoExistente} — confirmaste cargarla de todas formas (se permite, pero verificar)`);
    } else {
      errores.push(`Ya existe un registro con esta identificación (radicado ${contexto.radicadoExistente}). Usa "Editar" para decidir si la cargas de nuevas formas o vas a revisar la existente.`);
    }
  }

  // Regla de negocio: si la EPS viene como "Magisterio", se reemplaza
  // automáticamente por "NO SUMINISTRADA" (no aplica para este cargue).
  if (registro.eps && normalizarTexto(registro.eps) === normalizarTexto("MAGISTERIO")) {
    registro.eps = "NO SUMINISTRADA";
  }

  // Regla de negocio: el valor de salario siempre es 1.750.905, sin excepción.
  registro.valor_salario = "1750905";

  // Regla de negocio: el documento de identificación nunca lleva espacios.
  if (registro.identificacion) registro.identificacion = String(registro.identificacion).replace(/\s+/g, "");
  ...
  CAMPOS_FORMULARIO.forEach(c => {
    ...
    if (CAMPOS_FECHA_ETIQUETAS[clave] && (registro._fechasInvalidas || []).includes(clave)) return; // se reporta abajo con mensaje específico
    if (!req(registro[clave])) errores.push(`Falta ${c.etiqueta}`);
  });

  (registro._fechasInvalidas || []).forEach(clave => {
    errores.push(`${CAMPOS_FECHA_ETIQUETAS[clave]} no cumple el formato día/mes/año (ej. 29/06/2003)`);
  });
  ...
  if (registro.codigo_sucursal) {
    const sucursal = REF.ref_sucursales.find(s =>
      String(s.codigo) === String(registro.codigo_sucursal).trim() || normalizarTexto(s.nombre) === normalizarTexto(registro.codigo_sucursal)
    );
    if (sucursal) {
      registro.codigo_sucursal = String(sucursal.codigo); // siempre queda guardado el código (1 o 6), nunca texto libre
    } else {
      errores.push('Código de sucursal no válido (debe ser 1-CLAUSTRO o 6-HOSPITALES)');
    }
  }
...
function construirFilaEdicion(idx) {
  ...
  const controles = campos.map(c => {
    ...
    } else if (c.tipo === "select-sucursal") {
      html = `<select id="${idc}"><option value="">—</option>${REF.ref_sucursales.map(o =>
        `<option value="${escapeAttr(o.codigo)}" ${String(o.codigo) === String(valor).trim() || normalizarTexto(o.nombre) === normalizarTexto(valor) ? "selected" : ""}>${escapeAttr(o.codigo)} - ${escapeAttr(o.nombre)}</option>`).join("")}</select>`;
  ...
  const radicadoExistente = r.identificacion ? identificacionesEnBD.get(String(r.identificacion)) : null;
  const avisoDuplicado = radicadoExistente ? `
    <div class="aviso-duplicado">
      <strong>Ya existe un registro con esta identificación</strong> (radicado <strong>${escapeAttr(radicadoExistente)}</strong>).
      ¿Qué quieres hacer?
      <div class="aviso-duplicado-acciones">
        <button type="button" class="secundario btn-ver-existente" data-radicado="${escapeAttr(radicadoExistente)}">Ir a ver el registro existente</button>
        <button type="button" class="primario btn-confirmar-duplicado" data-idx="${idx}">Cargar de todas formas (registro nuevo)</button>
      </div>
    </div>` : "";

  return `<tr class="fila-edicion" data-fila-edicion="${idx}"><td colspan="10">
      ${avisoDuplicado}
...
function aplicarEdicionFila(idx) {
  ...
  campos.forEach(c => {
    ...
    // El input de fecha del editor siempre entrega formato válido (AAAA-MM-DD),
    // así que si este campo venía marcado con formato inválido, ya se resolvió.
    if (r._fechasInvalidas) r._fechasInvalidas = r._fechasInvalidas.filter(f => f !== c.clave);
  });
  validarYMostrarMasivo();
...
function validarYMostrarMasivo() {
  ...
  // Identificaciones que aparecen más de una vez dentro de este mismo archivo
  const conteoIds = {};
  filasMasivas.forEach(r => {
    if (r.identificacion) conteoIds[r.identificacion] = (conteoIds[r.identificacion] || 0) + 1;
  });

  let validas = 0, invalidas = 0;
  const filasHtml = filasMasivas.map((r, idx) => {
    const contexto = {
      duplicadoEnArchivo: !!(r.identificacion && conteoIds[r.identificacion] > 1),
      yaExisteEnBD: !!(r.identificacion && identificacionesEnBD.has(String(r.identificacion))),
      radicadoExistente: r.identificacion ? identificacionesEnBD.get(String(r.identificacion)) : null,
    };
    const errores = validarFilaMasiva(r, contexto);
    r._errores = errores;
  ...
  document.querySelectorAll(".btn-editar-fila").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Los botones de este editor recién se acaban de crear en el DOM, así
      // que se conectan aquí mismo (antes no existían cuando se intentaba
      // conectarlos más arriba, por eso "Aplicar cambios" no hacía nada).
      const filaEdicion = document.querySelector(`tr[data-fila-edicion="${idx}"]`);
      filaEdicion.querySelector(".btn-cancelar-edicion").addEventListener("click", () => filaEdicion.remove());
      filaEdicion.querySelector(".btn-aplicar-edicion").addEventListener("click", () => aplicarEdicionFila(Number(idx)));

      const btnConfirmarDup = filaEdicion.querySelector(".btn-confirmar-duplicado");
      if (btnConfirmarDup) {
        btnConfirmarDup.addEventListener("click", () => {
          filasMasivas[idx]._duplicadoConfirmado = true;
          filaEdicion.remove();
          validarYMostrarMasivo();
          mostrarToast("Confirmado: esta fila se guardará como un registro nuevo, aunque la identificación ya existía.", "ok");
        });
      }
      const btnVerExistente = filaEdicion.querySelector(".btn-ver-existente");
      if (btnVerExistente) {
        btnVerExistente.addEventListener("click", () => {
          document.querySelector('nav.tabs button[data-tab="registros"]').click();
          document.getElementById("filtro-texto").value = btnVerExistente.dataset.radicado;
...
document.getElementById("btn-guardar-masivo").addEventListener("click", async () => {
  ...
  filasMasivas.forEach((r, idx) => {
    if (r._errores.every(e => e.includes("se permite"))) {
      ...
      delete copia._fechasInvalidas;
      delete copia._duplicadoConfirmado;
...
document.getElementById("btn-exportar-xlsx").addEventListener("click", () => {
  ...
  const filas = seleccion.map(r => ([
    ...
    r.ocupacion_codigo, "0", r.codigo_sucursal, r.codigo_centro_trabajo, r.direccion, r.telefono, r.celular,
